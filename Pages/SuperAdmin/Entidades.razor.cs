...
namespace RestauranteVirtual.Web.Pages.SuperAdmin
{
	public partial class Entidades
	{
		...
		[Inject] EntidadService EntidadService { get; set; }
		...
		private int _EntidadIdPermisoEdicion = 0;
		private int _perfilIdEliminar = 0;
		public string _accion = "";
		private List<TreeNodeModel> _nodosPermisos = new();
		private List<EntidadDto> _todosElementos = new();
		public List<EntidadDto> _elementosPorPagina = new();
		private List<EntidadDto> _elementosFiltrados = new();
		...
		protected override async Task OnInitializedAsync()
		{
			...
			_elementosFiltrados = _todosElementos = await EntidadService.ObtenerEntidads();
		...
		public async void GuardarCambios()
		{
			...
			if (response.Success)
			{
				_todosElementos = await EntidadService.ObtenerEntidads();
				IdEdicion = 0;
				esNuevo = false;
				_mensajeError = "";
		...
		private async void CargarPermisos(int EntidadId)
		{
			_mensajeErrorPermisos = string.Empty;
			_EntidadIdPermisoEdicion = EntidadId;
			var permisos = await PerfilService.ListarPermisosPorEntidad(EntidadId);
		...
		private async void GuardarPermisos()
		{
			...
			{
				EntidadId = _EntidadIdPermisoEdicion,
				Permisos = permisosId.ToArray()
			};

			var response = await EntidadService.GuardarPermisos(request);
			if (response.Success)
			{
				_EntidadIdPermisoEdicion = 0;
				await JavascriptService.MostrarMensajeExito();
